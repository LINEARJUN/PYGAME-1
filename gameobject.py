from math import atan, sqrt, hypot
...
class Sprite():
    def __init__(self, image, size=None, update_ms=-1):
        self.size = size
        if size is not None:
            self.image = self.sheet = pygame.transform.scale(
                pygame.image.load(image), size)
        else:
            # use default size
...
class GameObject(Sprite):
    ...
    def dist_sq(this, other):      
        # hypot(x1 - x2, y1 - y2) 
        return hypot(this.pos[0]-other.pos[0], this.pos[1]-other.pos[1])

    @staticmethod
    def collide(this, other, r=None):
        # Return True when this and other collide. Otherwise, return False
        if this.dist_sq(this, other) <= this.radius + other.radius:
            return True
        else:
            return False
        pass
    ...
    def __init__(self, image, size, update_ms=-1, pos=(0, 0), velocity=(0, 0), acceleration=(0, 0)):
    ...
    def get_angle(self):
        self._last_angle = - \
            (180 * atan(self.velocity[1] / (1e-10 + self.velocity[0]))) / PI
        flip = False
...
class Marisa(GameObject):
    def __init__(self, image, size, update_ms, pos=(0, 0), velocity=(0.5, 0), acceleration=(0, 0)):
        ...
        self.size = [125, 70]
        self.normal_sprite = [pygame.Rect(
            8 + x * 125, 320, 125, 70) for x in range(6)]
        self.kurkuru_sprite = [pygame.Rect(
            8 + x * 125, 190, 125, 70) for x in range(8)]
